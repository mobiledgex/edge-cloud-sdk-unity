...
public class ComputerVision : MonoBehaviour
{
    ...
    /// <summary>
    /// Resizing the image before sending it to the server respecting aspect ratio
    /// </summary>
    ...
    bool webRequestsLock = true; // to control the flow of sending web requests
    ...
    IEnumerator Start()
    {
        ...
        {
            // Handle failure due internet connection through Wifi
            throw new Exception("MobiledgeX works with carrier data");
        }
    ...
    IEnumerator ImageSenderFlow()
    {
        ...
        texture.Apply();
        byte[] imgBinary = ShrinkAndEncode(source: texture, targetWidth: 240);
        Destroy(texture);
        while (!urlFound)
        {
            yield return null;
            ...
            case ServiceMode.ObjectDetection:
                foreach (@Object obj in objectsDetected)
                {
                    ...
                    if ((obj.confidence >= confidenceThreshold / 100 && highConfidenceTexture) || (obj.confidence < confidenceThreshold / 100 && lowConfidenceTexture))
                        GUI.DrawTexture(new Rect((obj.rect[0] * imgScalingFactor), (obj.rect[1] * imgScalingFactor), width, height), obj.confidence >= confidenceThreshold / 100 ? highConfidenceTexture : lowConfidenceTexture, ScaleMode.StretchToFill, true, width / height);
                    
                    TextStyle.font = objectClassFont;
                    TextStyle.normal.textColor = obj.confidence * 100 >= confidenceThreshold ? highConfidenceFontColor : lowConfidenceFontColor;
                    TextStyle.fontSize = objectClassFontSize;
                    string objClass = obj.@class + (showConfidenceLevel == true ? " " + obj.confidence * 100 + "%" : "");
                    if (objectClassFont)
                        GUI.Label(new Rect((obj.rect[0] * imgScalingFactor) + (width / 2) - (25), (obj.rect[1] * imgScalingFactor) + 10, 50, 50), new GUIContent(objClass), TextStyle);
                }
                break;
        }
    }
